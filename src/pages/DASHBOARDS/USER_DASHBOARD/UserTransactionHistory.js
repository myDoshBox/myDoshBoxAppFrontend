  ...
  return (
    <div className="contestPage" style={{ "background-color": "#F9F9FB" }}>
        ...
        export const RecentTransactionTable = () => {
          ...
          const [verifyEscrowProductTransactionPayment, { isLoading: verifyingEscrow }] =
          ...
          useEffect(() => {
            if (reference) {
              const verify = async () => {
                try {
                  const res = await verifyEscrowProductTransactionPayment(reference).unwrap();
          ...
          const { data: transactions, isLoading } = useFetchAllTransactionsQuery(userEmail, {
                    ...
                    <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                      ...
                      <div className="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2 ms-md-auto">
                        ...
                        <div className="d-flex gap-2 w-100 w-md-auto">
                          ...
                          <Link to="#" className="text-decoration-none flex-fill flex-md-grow-0">
                            <Button
                              className="btn-sm w-100 w-md-auto px-3 text-nowrap"
                              style={{ backgroundColor: '#198754', color: '#fff' }}
                            >
      ...
      <div className="table-responsive">
        <Table hover className="align-middle small w-100" style={{ fontFamily: 'monospace', fontSize: '13px' }}>
          ...
          <tbody>
            {getSlicedData().map((tx) => (
              <tr key={tx.transaction_id}>
                ...
                <td className="d-none d-sm-table-cell">{truncateText(tx.vendor_name)}</td>
                <td>{tx.createdAt?.slice(0, 10)}</td>
                <td className="d-none d-md-table-cell">
                  ₦{tx.transaction_type === 'buy' ? tx.transaction_total : tx.product_price}
                </td>
                <td className="d-none d-md-table-cell">{tx.transaction_type}</td>
                <td>
                  <span className={`badge px-2 py-1 fw-semibold ${tx.transaction_status.toLowerCase() === 'dispute' ? 'bg-warning text-dark' : 'bg-light text-success border border-success'}`}>
                ...
                <td>
                  <Button
                    ...
                    style={{ backgroundColor: 'white', border: '1px solid #198754', color: '#198754', whiteSpace: 'nowrap' }}
                    onMouseOver={(e) => {
      ...
      <Modal show={show} onHide={handleCloseModal} centered size="lg" dialogClassName="rounded-4">
        <Modal.Header closeButton className="bg-success text-white rounded-top-4">
            ...
            <div className="row g-4">
              <div className="col-md-6">
                <div className="p-3 border bg-white rounded-3 shadow-sm">
                  ...
                  <p className="small"><strong>Name:</strong> {selectedTransaction.product_name}</p>
                  <p className="small"><strong>Description:</strong> {selectedTransaction.product_description}</p>
                  <img src={selectedTransaction.product_image}
                            alt="Product" className="img-fluid rounded border" style={{ maxHeight: '200px' }}
                          />
              ...
              <div className="col-md-6">
                <div className="p-3 border bg-white rounded-3 shadow-sm">
                  <h6 className="fw-bold mb-3 text-success">Transaction Info</h6>
                  <p className="small"><strong>Type:</strong> {selectedTransaction.transaction_type}</p>
                  <p className="small"><strong>Status:</strong> {selectedTransaction.transaction_status}</p>
                  <p className="small"><strong>Total:</strong> ₦{selectedTransaction.transaction_total || selectedTransaction.product_price}</p>
                  <p className="small"><strong>Date:</strong> {selectedTransaction.createdAt?.slice(0, 10)}</p>
                  <p className="small"><strong>Time:</strong> {selectedTransaction.createdAt?.slice(11, 19)}</p>
                  <p className="small"><strong>Vendor:</strong> {selectedTransaction.vendor_name}</p>
                  <p className="small"><strong>Buyer:</strong> {selectedTransaction.buyer_email}</p>
                  <p className="small"><strong>Address:</strong> {selectedTransaction.delivery_address}</p>
                </div>
              </div>
            </div>
          ) : (
